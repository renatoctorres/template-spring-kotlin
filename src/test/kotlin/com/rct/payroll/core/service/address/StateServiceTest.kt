...
import com.rct.payroll.infra.persistence.entity.stub.StateStub
...
internal class StateServiceTest {
    private val repository: StateRepository = mockk(relaxed = true)
    private val service = StateService(repository = repository)

    @Nested
    inner class WhenCreatingState {
        @Test
        fun `it should use repository to persist state`() {
            val state: State = mockk()
            val expected: State = mockk()
            every { repository.save(state) } returns expected
            val actual = service.create(state)
    ...
    inner class WhenFindingById {
        ...
        @Test
        fun `it should find State when it exists`() {
            val expected: State = mockk(relaxed = true)
    ...
    @Nested
    inner class WhenUpdatingState {
        private val id = UUID.randomUUID()

        @Test
        fun `it should use repository to update state`() {
            val expected = StateStub.any()
    ...
    @Nested
    inner class WhenDeletingState {
        private val unit: Unit = mockk()

        @Test
        fun `it should use repository to delete state`() {
            val expected = StateStub.any()
            every { repository.findById(any()) } returns Optional.of(expected)
