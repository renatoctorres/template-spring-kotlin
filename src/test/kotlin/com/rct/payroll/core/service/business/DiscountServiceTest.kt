...
import com.rct.payroll.infra.persistence.entity.stub.DiscountStub
...
internal class DiscountServiceTest {
    private val repository: DiscountRepository = mockk(relaxed = true)
    private val service = DiscountService(repository = repository)

    @Nested
    inner class WhenCreatingDiscount {
        @Test
        fun `it should use repository to persist discount`() {
            val discount: Discount = mockk()
            val expected: Discount = mockk()
            every { repository.save(discount) } returns expected
            val actual = service.create(discount)
    ...
    inner class WhenFindingById {
        ...
        @Test
        fun `it should find Discount when it exists`() {
            val expected: Discount = mockk(relaxed = true)
    ...
    inner class WhenFindingAll {
        @Test
        fun `it should find Discount when it exists`() {
            val discount: Discount = mockk(relaxed = true)
            val expected = listOf(discount)
    ...
    @Nested
    inner class WhenUpdatingDiscount {
        private val id = UUID.randomUUID()

        @Test
        fun `it should use repository to update discount`() {
            val expected = DiscountStub.any()
    ...
    @Nested
    inner class WhenDeletingDiscount {
        private val unit: Unit = mockk()

        @Test
        fun `it should use repository to delete discount`() {
            val expected = DiscountStub.any()
            every { repository.findById(any()) } returns Optional.of(expected)
